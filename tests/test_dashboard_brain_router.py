...
@pytest.mark.asyncio
async def test_get_current_position_falls_back_to_broker_positions_when_local_missing():
    dashboard_state = DashboardState(current_price=101.0)
    ...
    async def _find_symbol_exchange(_symbol):
        return exchange, "mt5"

    exchange_manager.find_symbol_exchange = _find_symbol_exchange
    exchange_manager.exchanges = {"mt5": exchange}

    router = BrainRouter(
        config=SimpleNamespace(DATA_DIR="unused", CRYPTO_PAIR="XAU/USD"),
        logger=MagicMock(),
        dashboard_state=dashboard_state,
        vector_memory=None,
        unified_parser=None,
    ...
    assert result["has_position"] is True
    assert result["position_source"] == "broker_mt5"
...
@pytest.mark.asyncio
async def test_get_current_position_adds_sync_warning_when_broker_has_multiple_positions():
    position = Position(
        entry_price=83.03,
        stop_loss=82.50,
        take_profit=84.20,
        size=0.1,
        entry_time=MagicMock(isoformat=MagicMock(return_value="2026-04-17T22:10:31+00:00")),
        confidence="HIGH",
        direction="LONG",
        symbol="CRUDOIL",
        sl_distance_pct=0.0064,
        tp_distance_pct=0.0140,
        rr_ratio_at_entry=2.2,
    )
    dashboard_state = DashboardState(current_price=83.05)
    ...
    async def _fetch_positions(_symbol=""):
        return [
            {"ticket": 1, "symbol": "CRUDOIL", "type": "buy", "volume": 0.1, "price_open": 83.03, "price_current": 83.05, "sl": 82.5, "tp": 84.2},
            {"ticket": 2, "symbol": "CRUDOIL", "type": "sell", "volume": 0.1, "price_open": 83.20, "price_current": 83.05, "sl": 83.8, "tp": 82.4},
        ]
    ...
    router = BrainRouter(
        config=SimpleNamespace(DATA_DIR="unused", CRYPTO_PAIR="CRUDOIL", MT5_ENABLED=True),
        logger=MagicMock(),
        dashboard_state=dashboard_state,
        vector_memory=None,
        unified_parser=None,
    ...
    assert result["has_position"] is True
    assert result["position_source"] == "local_persistence"
    assert result["broker_position_count"] == 2
    assert "sync_warning" in result


@pytest.mark.asyncio
async def test_get_current_position_adds_sync_warning_when_local_exists_but_broker_empty_mt5():
    position = Position(
        entry_price=83.03,
        stop_loss=82.50,
        take_profit=84.20,
