...
from src.trading.data_models import (
    DebateArgument, DebateResult, Rating,
)
...
class DebateService:
    ...
    async def debate(
        ...
        """
        self.logger.info("Starting Bull/Bear debate for signal: %s (%s)", signal, confidence)
        ...
        # Run bull and bear arguments in parallel for speed
        bull_task = self._get_argument("bull", signal, confidence, analysis_context, reasoning)
        bear_task = self._get_argument("bear", signal, confidence, analysis_context, reasoning)
        bull_arg, bear_arg = await asyncio.gather(bull_task, bear_task)

        # Judge evaluates
        verdict = await self._judge(signal, confidence, analysis_context, bull_arg, bear_arg)

        self.logger.info(
            "Debate verdict: %s | Signal: %s→%s | Confidence: %s→%s | Δ%.0f",
            verdict.verdict, signal, verdict.final_signal,
    ...
    def _parse_verdict(
        ...
        if net_impact > 0.3:
            ...
            verdict = "NEUTRAL"
            final_confidence = "MEDIUM" if original_confidence == "HIGH" else original_confidence
