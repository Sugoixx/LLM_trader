...
class RagEngine:
    def __init__(
        ...
        Args:
            ...
            commodities_news_client: CommoditiesNewsClient for non-crypto assets (optional)
        """
        ...
        self.commodities_news_client = commodities_news_client

        # Active asset class (crypto/forex/oil/metals/macro) — used by dashboard
        # to label the news panel and by refresh_market_data() to route fetches.
        self.asset_class: str = getattr(config, 'ASSET_CLASS', 'crypto')
    ...
    async def refresh_market_data(self) -> None:
        ...
        # Fetch crypto news only when the active profile is crypto.
        # Forex/oil/metals profiles rely on CommoditiesNewsClient instead.
        if asset_class == 'crypto':
            try:
                crypto_articles = await self.news_manager.fetch_fresh_news(
                    self.ticker_manager.get_known_tickers()
                )
            ...
            self.logger.debug(
                "Non-crypto profile (%s): skipping CryptoCompare news fetch",
                asset_class,
            )

        # Fetch commodities / macro / forex news for non-crypto profiles only.
        if self.commodities_news_client and asset_class != 'crypto':
            try:
                symbol = getattr(self.config, 'CRYPTO_PAIR', None)
                commodity_articles = await self.commodities_news_client.fetch_news(
                    symbol=symbol,
                    asset_class=asset_class,
                )
                commodity_articles = self.commodities_news_client.filter_by_age(
                    commodity_articles, max_age_hours=48
                )
                articles.extend(commodity_articles)
                self.logger.info(
                    "Commodities RAG: %d articles fetched for %s (class=%s)",
                    len(commodity_articles), symbol or "general", asset_class,
                )
            except Exception as e:
                self.logger.error("Error fetching commodities news: %s", e)

        # Update market overview if needed (CoinGecko + DefiLlama — crypto-specific)
        if asset_class == 'crypto':
            try:
            ...
            self.logger.debug(
                "Non-crypto profile (%s): skipping CoinGecko/DefiLlama overview",
                asset_class,
            )
    ...
    async def get_market_overview(self) -> Optional[Dict[str, Any]]:
        ...
        if getattr(self.config, 'MT5_ENABLED', False):
            self.logger.debug("MT5 mode: skipping market overview fetch (CoinGecko/DefiLlama)")
            return None
        try:
