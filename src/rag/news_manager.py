...
class NewsManager:
    ...
    async def fetch_fresh_news(
        self, known_crypto_tickers: Set[str]
    ) -> List[Dict[str, Any]]:
        ...
        try:
            api_categories = (
                self.categories_api.get_api_categories()
                if self.categories_api
                else None
            )
            ...
            for article in articles:
                coins = self.article_processor.detect_coins_in_article(
                ...
                # Analyze sentiment
                self.article_processor.analyze_article_sentiment(article)

            self.logger.debug(
                "Fetched %s recent news articles from CryptoCompare", len(articles)
            )
            return articles
    ...
    def update_news_database(self, new_articles: List[Dict[str, Any]]) -> bool:
        ...
        recent = self.file_handler.filter_articles_by_age(
            new_articles, max_age_seconds=86400
        )
        ...
        self.news_database = self.file_handler.filter_articles_by_age(
            combined, max_age_seconds=86400
        )
        self.file_handler.save_news_articles(self.news_database)

        self.logger.debug(
            "Updated news database with %s recent articles", len(self.news_database)
        )
        return True
    ...
    def _normalize(self, article: Dict[str, Any]) -> None:
        ...
        article["detected_coins_str_lower"] = article.get(
            "detected_coins_str", ""
        ).lower()
