...
class BaseNotifier(ABC):
    ...
    @staticmethod
    def _source_prefix(position_or_source: Any) -> str:
        """Return a dashboard/title prefix for the position source slot.

        Usage::

            prefix = self._source_prefix(position)  # "[AI] " or "[FAST] "

        Accepts either a Position object (reads `.source`) or a raw string.
        Returns an empty string in single-slot mode (source unset / "ai"
        when DOUBLE_TRADE is off we still want "[AI]" for clarity in
        multi-slot UI, so we always emit the prefix when source is known).
        """
        source: Optional[str] = None
        if isinstance(position_or_source, str):
            source = position_or_source
        elif position_or_source is not None:
            source = getattr(position_or_source, "source", None)
        if not source:
            return ""
