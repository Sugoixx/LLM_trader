...
class ConsoleNotifier(BaseNotifier):
    ...
    async def send_position_status(
        ...
        try:
            ...
            print("=" * 60)
            print(f"Entry Price:     {self._fmt_price(position.entry_price)}")
            print(f"Current Price:   {self._fmt_price(current_price)}")
            ...
            print("-" * 40)
            print(f"Stop Loss:       {self._fmt_price(position.stop_loss)} ({stop_distance_pct:+.2f}%)")
            print(f"Take Profit:     {self._fmt_price(position.take_profit)} ({target_distance_pct:+.2f}%)")
    ...
    def _fmt_price(price) -> str:
        ...
        Uses 5 decimal places for low-value pairs (forex like EURUSD ~ 1.18)
        and 2 decimal places for higher-value instruments (crypto, oil, indices).
        Tolerant of list/tuple inputs (LLM occasionally returns multi-valued
        TP/SL) — unwraps the first numeric element.
        """
        # Unwrap single-element containers the LLM sometimes emits.
        if isinstance(price, (list, tuple)):
            price = next((p for p in price if isinstance(p, (int, float))), None)
        if price is None:
            return "n/a"
        try:
            price = float(price)
    ...
    def _print_analysis_data(self, analysis: dict, timeframe: str) -> None:
        ...
        try:
            ...
            if fields['entry_price']:
                print(f"Entry:           {self._fmt_price(fields['entry_price'])}")
            if fields['stop_loss']:
                print(f"Stop Loss:       {self._fmt_price(fields['stop_loss'])}")
            if fields['take_profit']:
                print(f"Take Profit:     {self._fmt_price(fields['take_profit'])}")
            if fields['risk_reward_ratio']:
                rr = fields['risk_reward_ratio']
                if isinstance(rr, (list, tuple)):
                    rr = next((x for x in rr if isinstance(x, (int, float))), None)
                try:
                    if rr is not None:
                        print(f"R:R Ratio:       {float(rr):.2f}")
                except (TypeError, ValueError):
                    pass
            ...
            if key_levels:
                ...
                if supports:
                    support_str = ", ".join([self._fmt_price(s) for s in supports[:3]])
                    print(f"Support:         {support_str}")
                if resistances:
                    resistance_str = ", ".join([self._fmt_price(r) for r in resistances[:3]])
