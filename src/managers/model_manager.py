...
class ModelManager(ModelManagerProtocol):
    ...
    async def send_prompt_streaming(
        ...
        effective_provider = provider if provider else self.provider
        if (
            (effective_provider in ("local", "all"))
            and self._clients.lmstudio
            and self.config.LM_STUDIO_STREAMING
        ):
            try:
                ...
                response_json = await self._clients.lmstudio.stream_chat_completion(
                    effective_model,
                    messages,
                    self._orchestrator.get_metadata("local").config,
                    callback=print_stream_callback,
                )
                if response_json is not None:
                    ...
                    return await self._process_result(result)
                self.logger.warning(
                    "LM Studio streaming returned None. Falling back to non-streaming mode."
                )
            except Exception as e:
                self.logger.warning(
                    "LM Studio streaming failed: %s. Falling back to non-streaming mode.",
                    str(e),
                )
    ...
    async def send_prompt_with_chart_analysis(
        ...
        messages = self._prepare_messages(prompt, system_message)
        effective_provider = provider if provider else self.provider
        result = await self._orchestrator.get_chart_response(
            effective_provider, messages, chart_image, model
        )
        if not result.success:
            raise ValueError(
                f"Chart analysis failed: {result.error or 'invalid response'}"
            )
        return await self._process_result(result)
    ...
    async def send_quick_prompt(
        ...
        """Send a prompt using a fast/cheap model for simple tasks.

        Uses the fallback model (typically smaller/cheaper) instead of the
        primary model. Ideal for: parsing, formatting, debate arguments,
        classification, and other tasks that don't require deep reasoning.

        Falls back to the primary model if no quick model is configured.
        ...
        Returns:
            Response text from the AI model
        """
        # Try to use the fallback model (typically cheaper/faster)
        quick_model = None
        effective_provider = self.provider

        if effective_provider == "openrouter" and self._clients.openrouter:
            quick_model = self.config.OPENROUTER_FALLBACK_MODEL
        elif effective_provider == "googleai" and self._clients.google:
            # Google free tier is already fast/cheap
            quick_model = self.config.GOOGLE_STUDIO_MODEL

        messages = self._prepare_messages(prompt, system_message)
        result = await self._orchestrator.get_text_response(
            effective_provider, messages, quick_model
        )
        return await self._process_result(result)
    ...
    def _prepare_messages(
        ...
        if system_message:
            ...
            self.logger.debug(
                "Pre-call estimate: system=%s, prompt=%s",
                f"{system_tokens:,}",
                f"{prompt_tokens:,}",
            )
    ...
    async def _process_result(self, result) -> str:
        ...
        if not result.success:
            ...
            return self.unified_parser.format_error_response(
    ...
    async def _track_cost(self, result, response, content: str) -> None:
        ...
        if result.provider == "openrouter" and self._clients.openrouter:
            ...
            if generation_id:
                cost_data = await self._clients.openrouter.get_generation_cost(
                    generation_id, retry_delay=0.8
                )
                if cost_data:
            ...
            if not is_free_tier:
                cost = self.model_pricing.get_cost(
                    "google", result.model, prompt_tokens, completion_tokens
                )
        ...
        self.cost_storage.record_usage(
            result.provider, prompt_tokens, completion_tokens, cost
        )
