...
class PersistenceManager:
    ...
    def save_position(self, position: Optional["Position"]) -> None:
        ...
        try:
            ...
            data = {
                ...
                "source": getattr(position, "source", "ai"),
            }
    ...
    # ────────────────────────────────────────────────────────────────────
    # Multi-slot persistence (AI + Fast). New format in positions.json:
    #     {"ai": {...}, "fast": {...}}
    # Legacy format (single flat object) is auto-migrated to {"ai": <old>}.
    # ────────────────────────────────────────────────────────────────────
    ...
    def _position_to_dict(position: "Position") -> Dict[str, Any]:
        return {
            ...
            "direction": position.direction,
            "symbol": position.symbol,
            "confluence_factors": [[n, s] for n, s in position.confluence_factors],
            "entry_fee": position.entry_fee,
            "size_pct": position.size_pct,
            "quote_amount": position.quote_amount,
            "atr_at_entry": position.atr_at_entry,
    ...
    def load_positions(self) -> Dict[str, "Position"]:
        """Load all active position slots from disk.

        Returns a dict keyed by source (``"ai"``, ``"fast"``). If the on-disk
        file is still in the legacy flat format, auto-migrates to
        ``{"ai": <position>}`` and logs at INFO level.
        """
        if not self.positions_file.exists():
            return {}
        ...
        try:
            ...
            self.logger.error("Error loading positions: %s", e)
            return {}

        if not isinstance(raw, dict):
            self.logger.error("positions.json: unexpected format, ignoring")
            return {}

        result: Dict[str, "Position"] = {}

        # Detect legacy format: flat object with 'entry_price' at top level
        if "entry_price" in raw and "ai" not in raw and "fast" not in raw:
            try:
                pos = self._dict_to_position(raw)
                pos.source = "ai"
                result["ai"] = pos
                self.logger.info(
                    "[persistence] Migrated legacy single position → slot 'ai'"
                )
                # Save back in new format so the migration is persistent
                try:
                    self.save_positions(result)
                except Exception:  # pylint: disable=broad-exception-caught
                    pass
                return result
            except Exception as e:  # pylint: disable=broad-exception-caught
                self.logger.error("[persistence] Legacy migration failed: %s", e)
                return {}

        # New format: {"ai": {...}, "fast": {...}}
        for source_key, pos_data in raw.items():
            if source_key not in ("ai", "fast"):
                self.logger.warning(
                    "[persistence] Unknown position slot '%s' — ignored", source_key
                )
                continue
            ...
            try:
                ...
                pos.source = source_key
                result[source_key] = pos
            except Exception as e:  # pylint: disable=broad-exception-caught
                self.logger.error(
                    "[persistence] Failed to load slot '%s': %s", source_key, e
                )

        return result

    def save_positions(self, positions: Dict[str, "Position"]) -> None:
        """Save all active position slots to disk (atomic replace)."""
        try:
            if not positions:
                if self.positions_file.exists():
                    self.positions_file.unlink()
                # Invalidate single-slot legacy cache
                self._position_cache = None
                self._position_cache_valid = True
                return

            data = {
                src: self._position_to_dict(pos) for src, pos in positions.items()
            }
            data = serialize_for_json(data)
            ...
            # Update the legacy cache for backward-compat read paths:
            # prefer AI slot, fall back to Fast.
            self._position_cache = positions.get("ai") or positions.get("fast")
            self._position_cache_valid = True

        except Exception as e:  # pylint: disable=broad-exception-caught
            self.logger.error("Error saving positions: %s", e)

    async def async_save_positions(
        self, positions: Dict[str, "Position"]
    ) -> None:
        """Non-blocking save_positions: runs on a thread-pool worker."""
        await asyncio.to_thread(self.save_positions, positions)
