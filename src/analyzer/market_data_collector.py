...
class MarketDataCollector:
    ...
    async def fetch_ohlcv(self, context) -> bool:
        ...
        try:
            ...
            # Fetch and process market sentiment data (Alternative.me Fear & Greed — crypto-specific)
            # Skip in MT5 mode (Forex/Commodities have no relevant Fear & Greed index)
            is_mt5 = getattr(self.rag_engine.config, 'MT5_ENABLED', False)
            if not is_mt5:
                await self.fetch_and_process_sentiment_data(context)
            else:
                self.logger.debug("MT5 mode: skipping Fear & Greed index fetch")
                context.sentiment = {}
