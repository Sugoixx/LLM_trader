...
def _detect_has_open_position(position_context: Optional[str]) -> bool:
    """Return True when ``position_context`` describes an actual open position.

    ``TradingStrategy.get_position_context`` emits ``"CURRENT POSITION: None"``
    when no slot is filled. Any other shape (including an absent string) is
    treated conservatively: missing context → assume position MAY exist (True)
    to avoid wrongly stripping UPDATE/CLOSE from the prompt on error paths.
    """
    if not position_context:
        return True
    return "CURRENT POSITION: None" not in position_context
...
class AnalysisEngine:
    ...
    def __init__(
        ...
        Args:
            ...
            signal_layer: Optional StrategySignalLayer for algo strategy signals
        """
        ...
        self.last_algo_signals: Optional[Dict[str, Any]] = None  # Structured algo signals for dashboard
    ...
    async def _enrich_market_context(self, current_ticker: Optional[Dict[str, Any]] = None) -> None:
        ...
        """
        is_mt5 = getattr(self.config, 'MT5_ENABLED', False)

        # Fetch market overview (CoinGecko + DefiLlama — crypto-specific, skip in MT5 mode)
        if not is_mt5:
            try:
            ...
            self.logger.debug("MT5 mode: skipping crypto market overview (CoinGecko/DefiLlama)")
            self.context.market_overview = {}
        ...
        # Fetch cryptocurrency details (CryptoCompare — crypto-specific, skip in MT5 mode)
        if self.market_api and not is_mt5:
            try:
            ...
            self.logger.debug("MT5 mode: skipping CryptoCompare coin details")
            self.context.coin_details = {}
    ...
    async def _generate_ai_analysis(
        ...
        system_prompt = self.prompt_builder.build_system_prompt(
            ...
            has_open_position=_detect_has_open_position(position_context),
        )
        # Build algo strategy signals section (classical strategies alongside AI)
        algo_signals: Optional[str] = None
        if self.signal_layer is not None and self.context.ohlcv_candles is not None:
            try:
                raw_result = self.signal_layer.run(
                    self.context.ohlcv_candles, self.symbol or ""
                )
                if raw_result is not None:
                    from datetime import datetime, timezone  # pylint: disable=import-outside-toplevel
                    self.last_algo_signals = {**raw_result, "timestamp": datetime.now(timezone.utc).isoformat()}
                    algo_signals = self.signal_layer._format_section(  # pylint: disable=protected-access
                        raw_result["signals"], raw_result["market_condition"]
                    )
            except Exception as _sig_err:  # pylint: disable=broad-exception-caught
                self.logger.warning("AlgoStrategySignalLayer failed: %s", _sig_err)
