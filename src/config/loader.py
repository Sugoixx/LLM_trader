...
class Config:
    ...
    def _load_environment(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"Error loading environment file {KEYS_ENV_PATH}: {e}"
            ) from e
    ...
    def _load_ini_config(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"Error loading configuration file {self._config_path}: {e}"
            ) from e
    ...
    def _build_dynamic_urls(self):
        ...
        self.RAG_NEWS_FILTER_SOURCES = self.get_config(
            "rag", "news_filter_sources", True
        )
        self.RAG_NEWS_ALLOWED_FEEDS = self.get_config("rag", "news_allowed_feeds", None)
        self.RAG_CATEGORIES_API_URL = (
    ...
    def _build_model_configs(self):
        ...
        default_max_tokens = self.get_config("ai_providers", "max_tokens", None)
        if default_max_tokens is None:
            raise RuntimeError(
                "`max_tokens` is required in [ai_providers] of config.ini"
            )

        self._default_model_config = {
            "temperature": self.get_config("ai_providers", "temperature", None),
            "top_p": self.get_config("ai_providers", "top_p", None),
            "top_k": self.get_config("ai_providers", "top_k", None),
            "freq_penalty": self.get_config("ai_providers", "freq_penalty", None),
            "pres_penalty": self.get_config("ai_providers", "pres_penalty", None),
            "max_tokens": default_max_tokens,
        }

        google_max_tokens = self.get_config("ai_providers", "google_max_tokens", None)

        # Only enforce Google config if we are actually using it
        if google_max_tokens is None and self.PROVIDER in ("googleai", "all"):
            raise RuntimeError(
                "`google_max_tokens` is required in [ai_providers] of config.ini when using Google models"
            )

        self._google_model_config = {
            "temperature": self.get_config("ai_providers", "google_temperature", None),
            "top_p": self.get_config("ai_providers", "google_top_p", None),
            "top_k": self.get_config("ai_providers", "google_top_k", None),
            "max_tokens": google_max_tokens,
            "thinking_level": self.get_config(
                "ai_providers", "google_thinking_level", "high"
            ),
            "google_code_execution": self.get_config(
                "ai_providers", "google_code_execution", False
            ),
        }
    ...
    def ADMIN_USER_IDS(self):
        ...
        if isinstance(admin_ids, (list, tuple)):
            ...
            for raw_id in admin_ids:
                try:
                    ...
                    logging.warning(
                        "Invalid ADMIN_USER_IDS entry '%s' in keys.env. Expected integers.",
                        raw_id,
                    )
                    return []
            return parsed_ids
        ...
        if isinstance(admin_ids, str):
            try:
                ...
                logging.warning(
                    "Invalid ADMIN_USER_IDS format in keys.env. Expected comma-separated integers."
                )
                return []
    ...
    @property
    def ALL_PROVIDER_ORDER(self) -> list:
        """Ordered list of providers to try when provider = 'all'.

        Parsed from [ai_providers] all_provider_order (comma-separated).
        Default priority: local (free/no rate-limit) → googleai → openrouter → blockrun.
        """
        raw = self.get_config(
            "ai_providers", "all_provider_order", "local,googleai,openrouter,blockrun"
        )
    ...
    def LM_STUDIO_BASE_URL(self):
        return self.get_config(
            "ai_providers", "lm_studio_base_url", "http://localhost:1234/v1"
        )
    ...
    def OPENROUTER_BASE_URL(self):
        return self.get_config(
            "ai_providers", "openrouter_base_url", "https://openrouter.ai/api/v1"
        )

    @property
    def OPENROUTER_BASE_MODEL(self):
        return self.get_config(
            "ai_providers", "openrouter_base_model", "google/gemini-2.5-pro"
        )

    @property
    def OPENROUTER_FALLBACK_MODEL(self):
        return self.get_config(
            "ai_providers", "openrouter_fallback_model", "deepseek/deepseek-r1:free"
        )

    @property
    def GOOGLE_STUDIO_MODEL(self):
        return self.get_config(
            "ai_providers", "google_studio_model", "gemini-2.5-flash"
        )

    @property
    def BLOCKRUN_BASE_URL(self):
        return self.get_config(
            "ai_providers", "blockrun_base_url", "https://blockrun.ai/api"
        )
    ...
    @property
    def LOG_DIR(self):
        return self.get_config("debug", "log_dir", "logs")

    @property
    def DATA_DIR(self):
        return self.get_config("debug", "data_dir", "data")
    ...
    def FILE_MESSAGE_EXPIRY(self):
        ...
        hours = self.get_config("safety", "file_message_expiry", 168)
        return hours * 3600
    ...
    def RAG_COINGECKO_GLOBAL_API_URL(self):
        return self.get_config(
            "rag", "coingecko_global_api_url", "https://api.coingecko.com/api/v3/global"
        )
    ...
    def SUPPORTED_EXCHANGES(self):
        ...
        return self.get_config("crypto", "supported", ["binance", "kucoin", "gateio"])

    @property
    def MARKET_REFRESH_HOURS(self):
        return self.get_config("crypto", "market_refresh_hours", 24)

    # Demo Trading Configuration
    @property
    def TRANSACTION_FEE_PERCENT(self):
        """Transaction fee percentage for limit orders (default 0.075%)."""
        return float(self.get_config("trading", "transaction_fee_percent", 0.00075))

    @property
    def DEMO_QUOTE_CAPITAL(self):
        """Initial capital for demo trading (default 10000)."""
        return float(self.get_config("trading", "demo_quote_capital", 10000.0))

    # Debate Configuration
    @property
    def DEBATE_ENABLED(self):
        """Enable Bull/Bear debate service (default True)."""
        return self.get_config("trading", "debate_enabled", True)

    @property
    def DEBATE_USE_QUICK_MODEL(self):
        """Use quick model for debate arguments (default True)."""
        return self.get_config("trading", "debate_use_quick_model", True)

    @property
    def DEBATE_COMBINED_ARGUMENTS(self):
        """Use combined bull/bear prompt to reduce API calls (default True)."""
        return self.get_config("trading", "debate_combined_arguments", True)

    @property
    def DEBATE_SKIP_FOR_HOLD(self):
        """Skip debate for HOLD/CLOSE signals (default True)."""
        return self.get_config("trading", "debate_skip_for_hold", True)

    # Backtest Configuration
    @property
    def BACKTEST_INITIAL_CAPITAL(self):
        """Initial capital for backtesting (default 10000)."""
        return float(self.get_config("trading", "initial_capital", 10000.0))

    # AI Safety Gates (applied in trading_strategy._open_new_position) --------
    @property
    def AI_MIN_CONFIDENCE(self):
        """Minimum confidence level for new AI entries (HIGH/MEDIUM/LOW)."""
        return (
            str(self.get_config("safety", "min_confidence", "MEDIUM")).upper().strip()
        )

    @property
    def AI_BLOCKED_DEBATE_VERDICTS(self):
        """List of debate verdicts that block new AI entries."""
        verdicts_str = self.get_config("safety", "blocked_debate_verdicts", "NEUTRAL")
        return [v.strip().upper() for v in str(verdicts_str).split(",")]

    @property
    def AI_CONSECUTIVE_LOSS_THRESHOLD(self):
        """Pause AI entries after N consecutive losses (0 = disabled)."""
        return int(self.get_config("safety", "consecutive_loss_threshold", 3))

    @property
    def AI_CONSECUTIVE_LOSS_COOLDOWN_MINUTES(self):
        """Cooldown duration (minutes) after losing-streak trip."""
    ...
    def EXECUTION_TRAILING_ATR_MULT(self):
        ...
        return float(
            self.get_config("execution_engine", "trailing_atr_multiplier", 2.0)
        )

    @property
    def EXECUTION_TRAILING_ATR_MULT_AFTER_TP1(self):
        """ATR multiplier after TP1 breakeven activation (default 0.2)."""
        return float(
            self.get_config(
                "execution_engine", "trailing_atr_multiplier_after_tp1", 0.2
            )
        )
    ...
    def LIVE_MAX_ORDER_USD_FAST(self):
        ...
        val = self.get_config("safety", "max_order_usd_fast", None)
        if val is None or val == "":
            return self.LIVE_MAX_ORDER_USD
        return float(val)
    ...
    def DOUBLE_TRADE_ENABLED(self):
        ...
        val = self.get_config("safety", "double_trade_enabled", False)
    ...
    def MAX_MIN_LOT_MARGIN_PCT(self):
        ...
        setups: 0.05 to 0.15.
        """
        val = self.get_config("safety", "max_min_lot_margin_pct", 0.10)
        try:
    ...
    def LIVE_CONFIRM_ORDERS(self):
        ...
        return self.get_config("safety", "confirm_orders", True)

    @property
    def LIVE_TESTNET(self):
        """Use exchange testnet/sandbox (Binance demo-api.binance.com). Default False."""
        return bool(self.get_config("crypto", "testnet", False))
    ...
    def DEFAULT_TAKE_PROFIT_PCT(self):
        ...
        return float(
            self.get_config("risk_management", "default_take_profit_pct", 0.04)
        )
    ...
    def get_model_config(
        self, model_name: str, overrides: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
