...
class ExchangeManager:
    ...
    async def _load_exchange(self, exchange_id: str) -> Optional[ccxt.Exchange]:
        ...
        try:
            ...
            # Inject API credentials for the live-trading exchange.
            # IMPORTANT: This ExchangeManager is used exclusively for public
            # market data (OHLCV, tickers). Injecting testnet keys here while
            # the exchange runs against mainnet causes every request to be
            # signed with keys the mainnet doesn't know → "Invalid Api-Key ID"
            # (Binance code -2008). Skip credentials when sandbox mode differs
            # from the data endpoint (i.e. when testnet is active).
            live_enabled = bool(getattr(self.config, 'LIVE_TRADING_ENABLED', False))
            live_exchange = getattr(self.config, 'LIVE_EXCHANGE', None) or 'binance'
            use_testnet = bool(getattr(self.config, 'LIVE_TESTNET', False))
            is_live_exchange = live_enabled and exchange_id == live_exchange
            if is_live_exchange and not use_testnet:
                api_key = getattr(self.config, 'BINANCE_API_KEY', None)
                api_secret = getattr(self.config, 'BINANCE_API_SECRET', None)
                if api_key:
            ...
            # NOTE: We intentionally do NOT apply set_sandbox_mode() here.
            # The ExchangeManager is used exclusively for market data (OHLCV, tickers,
            # market discovery). Sandbox/testnet routing is applied only to the dedicated
            # LiveExecutor exchange (created in start.py._create_order_executor).
            # Routing the data exchange to testnet limits historical data to a few days,
            # causing "fewer closed candles than expected" warnings.
            if is_live_exchange and use_testnet:
                self.logger.info(
                    "[Exchange] %s market-data exchange uses mainnet (OHLCV history). "
                    "Order execution will use testnet sandbox.",
                    exchange_id,
                )
