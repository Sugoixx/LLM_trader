...
class PerformanceRouter:
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        # Override with live MT5 balance when available
        live_cap = getattr(self.dashboard_state, "live_capital", None)
        if isinstance(live_cap, (int, float)) and live_cap > 0:
            result["current_capital"] = live_cap
            if result.get("total_trades", 0) == 0:
                result["initial_capital"] = live_cap
