...
import { initPositionPanel, updatePositionData } from './modules/position_panel.js?v=6.0';
...
import { initNewsPanel, updateNewsData } from './modules/news_panel.js?v=6.8';
import { initAutoTrade } from './modules/auto_trade.js?v=6.0';
import { initManualTrade, refreshPositionState } from './modules/manual_trade.js?v=6.0';
import { initSettingsPanel } from './modules/settings_panel.js?v=6.5';
...
async function fetchBrainStatus() {
    try {
        ...
        renderMarketHoursBadge(data.market_hours);
...
function renderMarketHoursBadge(marketHours) {
    const badge = document.getElementById('market-hours-badge');
    if (!badge) return;

    if (!marketHours || marketHours.enabled === false) {
        badge.style.display = 'none';
        badge.className = 'market-hours-badge';
        return;
    }

    let text = 'Market Open';
    let badgeClass = 'open';

    if (marketHours.state === 'preopen_analysis') {
        text = 'Pre-open analysis running';
        badgeClass = 'preopen';
    } else if (marketHours.state === 'closed_preopen_done') {
        text = 'Market closed - pre-open done';
        badgeClass = 'paused';
    } else if (marketHours.llm_paused || marketHours.state === 'closed') {
        text = 'Market closed - LLM paused';
        badgeClass = 'paused';
    }

    const reason = marketHours.reason ? `Reason: ${marketHours.reason}` : '';
    const nextOpen = marketHours.next_open_utc ? `Next open: ${marketHours.next_open_utc}` : '';

    badge.textContent = text;
    badge.title = [reason, nextOpen].filter(Boolean).join(' | ');
    badge.className = `market-hours-badge ${badgeClass}`;
...
function initApp() {
    ...
    // Initialize each module independently — a failure in one must not block the others
    safeInit('PerformanceChart', initPerformanceChart);
    safeInit('SynapseNetwork', initSynapseNetwork);
    safeInit('VectorPanel', initVectorPanel);
    safeInit('Fullscreen', initFullscreen);
    safeInit('PositionPanel', initPositionPanel);
    safeInit('StatisticsPanel', initStatisticsPanel);
    safeInit('NewsPanel', initNewsPanel);
    safeInit('AlgoPanel', initAlgoPanel);
    safeInit('WebSocket', initWebSocket);
    safeInit('AutoTrade', initAutoTrade);
    safeInit('ManualTrade', initManualTrade);
    safeInit('SettingsPanel', initSettingsPanel);
    safeInit('UI', initUI);
