...
class DashboardServer:
    ...
    def __init__(self,
        ...
        self.trading_strategy = None  # Injected post-init via set_trading_strategy()
        self._execution_router = None
        self._settings_router = None
        self.bot = None  # Injected post-init via set_bot()
    ...
    def _create_app(self) -> FastAPI:
        ...
        execution_router = execution.ExecutionRouter(
            config=self.config,
            logger=self.logger,
            dashboard_state=self.dashboard_state,
        ...
        settings_router = settings.SettingsRouter(
            config=self.config,
            logger=self.logger,
            dashboard_state=self.dashboard_state,
        )
    ...
    def set_trading_strategy(self, trading_strategy):
        """Inject trading_strategy after construction (avoids circular deps)."""
        self.trading_strategy = trading_strategy
        if self._execution_router:
            self._execution_router.trading_strategy = trading_strategy

    def set_bot(self, bot):
        """Inject the running bot so symbol-switch endpoints can reach it."""
        self.bot = bot
        if self._settings_router:
            self._settings_router.bot = bot
